...
import com.github.tukcps.sysmd.ast.AstBinOp
...
import com.github.tukcps.sysmd.services.AgilaSessionImpl
...
class ParserSysML(
     ...
     */
    private fun parseBody(namespace: Namespace) {
        when (token.kind) {
            SEMICOLON -> {
                nextToken()
                return
     ...
     */
    private fun <T> optionalWhen(start: Token.Kind, default: T, rule: ParserSysML.() -> T): T =
        if (token.kind == start) {
            rule()
        } else
            default
    ...
    private fun <T> ifToken(start: Token.Kind, default: T? = null, rule: ParserSysML.() -> T): T? =
        if (token.kind == start) {
            rule()
        } else
            default
