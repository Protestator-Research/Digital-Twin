...
class SysMdSemantics(
     ...
     * @param identification The name of the Classifiable
     * @param superclass The qualified name of the superclass of the Classifiable
     * @param sources A feature that specifies class of the sources.
     * @param targets A feature that specifies class of the targets.
     * @param owner The qualified name of the owner of the Classifiable
    ...
    fun isARelationship(
        ...
        sources: Feature?,
        sourcesType: Identity<Type>?,
        sourcesMult: IntegerRange?,
        targets: Feature?,
        targetsType: Identity<Type>?,
        targetsMult: IntegerRange?,
        owner: QualifiedName
    ) {
        val sourcesOptName = mutableListOf<Identity<Element>>()
        if (sources != null) {
            sourcesOptName.add(Identity<Type>(id=sources.elementId, ref=sources, str = "from"))
        }
        val targetsOptName = mutableListOf<Identity<Element>>()
        if (targets != null)
            targetsOptName.add(Identity<Type>(id=targets.elementId, ref=targets, str = "to"))
        ...
        if (sources != null && sourcesType != null) {
            ...
            model.addUnownedElement(fromMultiplicity, toEffectiveName(owner) + "::${association.effectiveName}")
            model.addUnownedElement(SpecializationImplementation(fromMultiplicity, "ScalarValues::Integer"), toEffectiveName(owner) + "::${association.effectiveName}::from-multiplicity")
        ...
        if (targets != null && targetsType != null) {
            ...
            model.addUnownedElement(SpecializationImplementation(toMultiplicity, "ScalarValues::Integer"), toEffectiveName(owner) + "::${association.effectiveName}::to-multiplicity")
        ...
        objList.forEach { projectName ->
            if (subject == "Document" || subject == "Cell")
                try {
                    ...
                    model.report("in import of $projectName: " + (error.message?:"error in import"))
                }
            else
    ...
    fun connects(from: QualifiedName, to: QualifiedName, subject: Identification, namespace: Namespace) {
        ...
        model.create(connection, namespace)
    }
    ...
    fun associates(from: QualifiedName, to: QualifiedName, subject: Identification, namespace: Namespace) {
        ...
        model.create(association, namespace)
    }
     ...
     */
    internal fun ifElseExpression(condExpr: AstNode, thenExpr: AstNode, elseExpr: AstNode): AstIte {
