 ...
 */
internal class AstVectorAngle(model: AgilaSession, args: ArrayList<AstNode>) : AstFunction("norm", model, 1, args) {
    init {
        if (args.size !in 2..2)
            throw SemanticError("Angle expects 2 parameters of type Real or Integer Vector")
    }
    ...
    override fun initialize() {
        upQuantity = when (getParam(0).upQuantity.values[0]) {
            ...
            else -> throw SemanticError("Angle must have a Real or Integer argument")
        }
        evalUp()
