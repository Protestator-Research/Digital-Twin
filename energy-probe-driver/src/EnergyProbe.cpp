...
namespace ENERGY_PROBE_DRIVER {
	EnergyProbe::EnergyProbe(const char* outputPath, FILE* binfile, Fifo* fifo)
		: Device(outputPath, binfile, fifo)
	{
		mIsRunning = false;
	}
	...
	void EnergyProbe::processBuffer()
	{
		...
		while (location < inLength) {
			if (remaining == 0) {
				...
				value = (int)((float)value * gSessionData->getSourceScaleFactor(mNumFields - remaining));
				// Check for overflow
				if (value & ~0x7FFFFFFF) {
					value = 0x7FFFFFFF;
	...
	void EnergyProbe::prepareChannels()
	{
		...
		for (index = 0; index < MAX_COUNTERS; index++) {
			if (gSessionData->getEnableStateForCounter(index)) {
				if (!(mFields[gSessionData->getCounterChannelWithIndex(index)] & gSessionData->getCounterFieldWithIndex(index))) {
					// increment mNumFields if field not already accounted for
					mNumFields++;
				}
				// bitwise OR all types on a per channel basis
				mFields[gSessionData->getCounterChannelWithIndex(index)] |= gSessionData->getCounterFieldWithIndex(index);
			}
		}
