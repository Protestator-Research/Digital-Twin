...
#include "SysMLv2ListenerImplementation.h"
...
void SysMLv2ListenerImplementation::enterPackage(SysMLv2Parser::PackageContext* )
{
...
void SysMLv2ListenerImplementation::exitAttribute_usage(SysMLv2Parser::Attribute_usageContext* ctx)
{
	if ((ctx->usage()->usage_declaration()->feature_specialization_part() == nullptr))
	{
		std::cout << "Issue while creating Attribute usage, type is needed." << std::endl;
	}
	auto type = ctx->usage()->usage_declaration()->feature_specialization_part()->feature_specilization()[0]->typings()->typed_by()->owned_feature_typing()->getText();
	auto name = ctx->usage()->usage_declaration()->identification()->getText();
...
void SysMLv2ListenerImplementation::exitIndividual_usage(SysMLv2Parser::Individual_usageContext* ctx)
{
	...
	const auto partDefinition = ctx->usage()->usage_declaration()->feature_specialization_part()->feature_specilization().front()->typings()->typed_by()->owned_feature_typing()->general_type()->getText();
...
void SysMLv2ListenerImplementation::enterPart_definition(SysMLv2Parser::Part_definitionContext*)
{
	auto component = new DigitalTwin::Model::Component("");
	ParentStack.emplace(component);
}

void SysMLv2ListenerImplementation::exitPart_definition(SysMLv2Parser::Part_definitionContext* ctx)
{
...
void SysMLv2ListenerImplementation::exitPart_usage(SysMLv2Parser::Part_usageContext* ctx)
{
	if (ParentStack.size() > 0)
	{
		const auto memberName = ctx->usage()->usage_declaration()->identification()->getText();
...
void SysMLv2ListenerImplementation::enterPort_definition(SysMLv2Parser::Port_definitionContext*)
{
	auto port = new DigitalTwin::Model::Port("");
	ParentStack.emplace(port);
}

void SysMLv2ListenerImplementation::exitPort_definition(SysMLv2Parser::Port_definitionContext* ctx)
{
...
void SysMLv2ListenerImplementation::exitPort_usage(SysMLv2Parser::Port_usageContext* ctx)
{
	if (ParentStack.size() > 0)
	{
		const auto memberName = ctx->usage()->usage_declaration()->identification()->getText();
		const auto portDefinition = ctx->usage()->usage_declaration()->feature_specialization_part()->feature_specilization().front()->typings()->typed_by()->owned_feature_typing()->general_type()->getText();
...
std::vector<DigitalTwin::Model::IDigitalTwinElement*> SysMLv2ListenerImplementation::getElements() const
{
	return Elements;
}

DigitalTwin::Model::IDigitalTwinElement* SysMLv2ListenerImplementation::resolveElement(std::string connectorEndMember)
{
...
DigitalTwin::Model::SupportedTypes SysMLv2ListenerImplementation::getTypeForString(std::string type)
{
...
DigitalTwin::Model::IDigitalTwinElement* SysMLv2ListenerImplementation::getElementWithName(std::string name)
{
