...
namespace PHYSICAL_TWIN_COMMUNICATION {
    ...
    inline void close_proc(std::set < con_sp_t > &cons, mi_sub_con & subs, con_sp_t
    const& con) {
    cons.
    erase(con);
...
CommunicationService::CommunicationService(std::string mqttPort) {
    ...
    auto s = MQTT_NS::server<>(
            boost::asio::ip::tcp::endpoint(
                    ...
                    boost::lexical_cast<std::uint16_t>(mqttPort)
            ),
            ioc
    );
                ...
                ep.set_subscribe_handler(
                        [&subs, wp]
                            ...
                            for (auto const &e: entries) {
                                std::cout << "[server] topic_filter: " << e.topic_filter << " qos: "
                                          << e.subopts.get_qos() << std::endl;
